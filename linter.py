...
class Standardrb(RubyLinter):
    ...
    def cmd(self):
        ...
        # Set tempfile_suffix so by default a tempfile is passed onto standardrb:
        self.tempfile_suffix = 'rb'

        path = self.filename
        if not path:
            # File is unsaved, and by default unsaved files use the default
            # standardrb config because they do not technically belong to a folder
            ...
            # inside this root folder, and standardrb will pick up on any
        ...
        if path:
            ...
            # and then tell standardrb to use this path (to search for config
            # files and to use for matching against configured paths - i.e. for
            # inheritance, inclusions and exclusions).
            #
            # The 'force-exclusion' overrides standardrb's behavior of ignoring
